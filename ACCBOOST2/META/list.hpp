#ifndef ACCBOOST2_META_LIST_HPP_
#define ACCBOOST2_META_LIST_HPP_
...
namespace ACCBOOST2::META
{
  ...
  /// T に合致する list の要素のインデックスを返すメタ関数
  ...
  template<class T, class List>
  constexpr bool in_v = META::in<T, List>::value;
  ...
  struct exists_duplicates<META::list<T, U...>>: std::bool_constant<META::in<T, META::list<U...>>::value || META::exists_duplicates<META::list<U...>>::value> {};
  ...
  namespace _product
  {
    ...
    template<class... T, class... Lists>
    struct impl<META::list<META::list<T...>, Lists...>>
    {
      using type = META::chain<typename META::_product::chain_for<T, typename META::_product::impl<META::list<Lists...>>::type>::type...>;
    };

  }

  template<class ListList>
  using product = typename META::_product::impl<ListList>::type;
  ...
  namespace _index_sequence
  {
    ...
    {
      using type = META::chain<
        typename META::_index_sequence::impl<N - 1>::type,
        META::list<std::integral_constant<std::size_t, N - 1>>
      >;
    };
  ...
  template<std::size_t N>
  using index_sequence = typename META::_index_sequence::impl<N>::type;
...
//    using function = typename META::negative<sub>::function<U>;
  };
  ...
  template<>
  struct all<META::list<>>: std::bool_constant<true>
  {};

  template<class T, class... U>
  struct all<META::list<T, U...>>: std::bool_constant<T::value && META::all<META::list<U...>>::value>
  {};
  ...
  template<>
  struct any<META::list<>>: std::bool_constant<false>
  {};

  template<class T, class... U>
  struct any<META::list<T, U...>>: std::bool_constant<T::value || META::any<META::list<U...>>::value>
  {};
  ...
  template<class T>
  struct max<META::list<T>>: T
  {};

  template<class T, class U, class... V>
  struct max<META::list<T, U, V...>>: META::max<META::list<std::conditional_t<(T::value > U::value), T, U>, V...>>
  {};

  template<class List>
  constexpr auto max_v = max<List>::value;
